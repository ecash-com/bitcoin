...
const char * const BITCOIN_CONF_FILENAME = "ecash.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\ecash
    //   new: C:\Users\Username\AppData\Local\ecash
    // macOS: ~/Library/Application Support/ecash
    // Unix-like: ~/.ecash
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "ecash";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "ecash";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/ecash";
#else
    // Unix-like
    return pathRet / ".ecash";
#endif
#endif
}
